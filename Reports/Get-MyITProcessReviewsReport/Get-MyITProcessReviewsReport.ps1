<#
    .SYNOPSIS
        Gets a review report from the MyITProcess reporting API.

    .DESCRIPTION
        The Get-MyITProcessReviewsReport script gets a review report from the MyITProcess reporting API.
    ...
    .PARAMETER Days
        Defines the number of days an active review has not had activity to be classified as inactive
    ...
    .EXAMPLE
        Get-MyITProcessReviewsReport -APIKey 12345

        Gets all reviews from the MyITProcess reporting API and sends the data to a CSV file.
        By default any active reviews that have not had activity in the past 90 days will be classified as inactive.

    .EXAMPLE
        Get-MyITProcessReviewsReport -APIKey 12345 -Days -180 -Report HTML

        Gets all reviews from the MyITProcess reporting API and sends the data to a HTMl file.
        Any active reviews that have not had activity in the past 180 days will be classified as inactive.

    .EXAMPLE
        Get-MyITProcessReviewsReport -APIKey 12345 -Days -30 -Report All

        Gets all reviews from the MyITProcess reporting API and sends the data to both a CSV & HTML file.
        Any active reviews that have not had activity in the past 30 days will be classified as inactive.
...
#Region    [ Prerequisites ]
    ...
    $ScriptName = 'Get-MyITProcessReviewsReport'
...
Write-Output " - (1/3) - $(Get-Date -Format MM-dd-HH:mm) - Getting reviews from MyITProcess"
...
#Region     [ MyITProcess API ]
    ...
    Do {
        $Current_Page = Get-MyITProcessReviews -sortingRules -sort_field_name client.name -paginationRule -page_number $Page_Number
    ...
    #Customize the results
        Try{
            $MyITProcessReviewResults = $PageResults | Select-Object `
                                            @{Name='id';Expression={$_.id}}, `
                                            @{Name='ReviewName';Expression={$_.name}}, `
                                            @{Name='Status';Expression={$_.status}}, `
                                            @{Name='InActive';Expression={  If ( ($_.status -eq 'New' -or $_.status -like "*InProgress*") -and (Get-Date).AddDays([Int]$Days) -gt $_.lastUpdatedDate){'Inactive'}
                                                                            Else{$null}}}, `
                                            @{Name='Client';Expression={$_.client.Name}}, `
                                            @{Name='AssignedvCIO';Expression={$_.AssignedvCIO.fullName}}, `
                                            @{Name='AssignedEngineer';Expression={$_.AssignedEngineer.fullName}}, `
                                            @{Name='createdDate';Expression={[DateTime]$_.createdDate | Get-Date -Format 'yyyy-MM-dd hh:mm'}}, `
                                            @{Name='lastUpdatedDate';Expression={[DateTime]$_.lastUpdatedDate | Get-Date -Format 'yyyy-MM-dd hh:mm'}}
        }
        Catch{
            Write-Host $_.Exception.Message -ForegroundColor Red -BackgroundColor Black
            ''
...
#Region     [ CSV Report ]
    ...
    Try{
        If($Report -eq 'All' -or $Report -eq 'CSV'){
            ...
            $MyITProcessReviewResults | Select-Object $ScriptName,* | Export-Csv $CSVReport -NoTypeInformation
        }
    }
    Catch{
        Write-Host $_.Exception.Message -ForegroundColor Red -BackgroundColor Black
...
#Region    [ HTML Report]
    ...
    Try{
        If ($Report -eq 'All' -or $Report -eq 'HTML'){
            ...
            $TotalReviews = ($MyITProcessReviewResults.id).count
            $ActiveReviews = ($MyITProcessReviewResults | Where-Object {$_.status -eq 'New' -or $_.status -like "*InProgress*"}).id.count
            $InActiveReviews = ($MyITProcessReviewResults | Where-Object {($_.status -eq 'New' -or $_.status -like "*InProgress*") -and (Get-Date).AddDays([Int]$Days) -gt $_.lastUpdatedDate}).count
            $AssignedvCIOs = ($MyITProcessReviewResults | Select-Object AssignedvCIO -Unique).count
            $AssignedEngineers = ($MyITProcessReviewResults | Select-Object AssignedEngineer -Unique).count
        ...
        #Region    [ HTML Report Building Blocks ]
            ...
            #HTML in Summary.html would be edited depending on the report and summary info you want to show
                ...
                $SummaryCards = $SummaryCards -replace ('xCARD1Valuex',$TotalReviews)
                $SummaryCards = $SummaryCards -replace ('xCARD2Valuex',"$ActiveReviews%")
                $SummaryCards = $SummaryCards -replace ('xCARD3Valuex',$InActiveReviews)
                $SummaryCards = $SummaryCards -replace ('xCARD4Valuex',$AssignedvCIOs)
                $SummaryCards = $SummaryCards -replace ('xCARD5Valuex',$AssignedEngineers)
        ...
        #Region    [ Example HTML Report Data\Structure ]
            ...
            $TableResults = $MyITProcessReviewResults | ConvertTo-Html -As Table -Fragment -Property status,InActive,ReviewName,Client,AssignedvCIO,AssignedEngineer,createdDate,lastUpdatedDate `
                                            -PostContent    '   <ul>
